...
- **2026-05-19 — Rich modern editor in advanced repeater:** **`advanced_repeater`** leaves may use **`rich_modern_editor`**; mount on row expand/add. **`sto-rich-modern-editor.js`** uses **`BlockList`** only (no extra **`DefaultBlockAppender`**). WC product-data + adv-rep CSS keep inputs/editor inside repeater cards (`min-width: 0`, `max-width: 100%`).
