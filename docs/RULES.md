...
- **2026-05-19 — Rich modern editor after tables:** **`BlockList`** **`renderAppender`** → **`DefaultBlockAppender`** (not a sibling node) so content can be added below tables; **`sto-rich-modern-editor.css`** uses **`overflow-y: visible`** + bottom padding on the block list.
